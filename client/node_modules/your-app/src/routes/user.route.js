...
router.get('/getUser',(req,res,next) =>{
    ...
    Students.find(req.params, function (err, result) {
...
router.get('/getStudentUser',(req,res,next) =>{
    ...
    Students.find(query, function (err, result) {
        if (err) return next(err);
        res.send(result);
    })
});
...
router.put('/updateProfile/:id', (req,res,next) =>{
    ...
    //     return res.status(400).json(errors);
    // }
    Students.findByIdAndUpdate(req.params.id, {$set: req.body}, function (err,result) {
        if (err) return next(err);
